...
class TaskFacade:
    def __init__(self, base_url=None, date_from=None, date_to=None, period=PeriodEnum.DAY,
                 folder_name=None,
                 csv_operation=CsvOutOpEnum.CONCATENATE, out_csv_path=None, picked_columns=None,
    ...
    def generate_schedule(self):
        ...
        if None not in (self.base_url,):
            task = [DownloadTask, {'base_url': self.base_url,
                                   ...
                                   'folder_path': self.__dict__.get('folder_path', None),
                                   'period': self.__dict__.get('period', PeriodEnum.WEEK),
                                   }]
        ...
        if None not in (self.csv_operation,):
            ...
            task = [cls, {'folder_path': self.__dict__.get('folder_path', None), 'picked_columns': self.picked_columns}]
            yield task
...
class LastWeekTaskFacade(TaskFacade):
    def __init__(self, base_url, current_date=None,
                 out_csv_path=None, csv_operation=CsvOutOpEnum.CONCATENATE, picked_columns=None,
                 gsheet_id=None, gsheet_cell_address=None):
        d1, d2 = get_previous_week_range(current_date)
        super().__init__(base_url=base_url, date_from=d1, date_to=d2, period=PeriodEnum.DAY,
                         csv_operation=csv_operation, out_csv_path=out_csv_path, picked_columns=picked_columns,
