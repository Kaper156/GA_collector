...
if __name__ == '__main__':
    ...
    if LEVEL_WORK >= LW_CHECK_FOLDER:
        ...
        try:
            print(f"Создаю папку: {os.path.abspath(FOLDER_NAME)}")
        ...
        with BrowserScenario(FOLDER_NAME, PROFILE_PATH, IS_AUTH_NEEDED) as bs:
            ...
            while len(undownloaded_urls):
                ...
                for url in undownloaded_urls:
                    ...
                    print(f"[{cnt} из {len(undownloaded_urls)}({len(urls)})] (Скачано {percent:002f}) \t"
                          f"Скачиваю csv за период {period[0]} - {period[1]}, по ссылке:{url}")
